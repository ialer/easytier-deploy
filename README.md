...
AXIBA-SN 虚拟网络一键部署工具，适用于 Windows 设备。
...
- **虚拟IP末位** — 10.126.126.X 的 X 值（每人分配不同）
...
```toml
[network_identity]
network_name = "AXIBA-SN"      # 网络名，所有节点相同
network_secret = "easytier9900" # 密钥，所有节点相同

[[peer]]
uri = "tcp://96.44.141.123:11010"  # 引导节点

[flags]
ipv4 = "10.126.126.5/24"   # 虚拟IP，每台设备不同
...
| IP | 用途 |
|----|------|
| 10.126.126.1 | 引导服务器（VPS） |
| 10.126.126.2 ~ 254 | 团队设备 |
...
```
设备A (10.126.126.2)  ──┐
设备B (10.126.126.3)  ──┼── VPS引导 (96.44.141.123) ── P2P直连/中继
设备C (10.126.126.4)  ──┘
```

所有设备连接到 VPS 引导节点，建立 P2P 直连或通过 VPS 中继通信。
