    ...
    try:
        with open(CONFIG_TOML, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if m:
                    return html_mod.escape(m.group(1))
...
function openCfg(){
  stopRefresh();
  fetch('/api/config',{headers:{'X-Auth-Token':_authToken}}).then(function(r){return r.text()}).then(function(t){
...
function saveCfg(){
  ...
  if(txt.length>100000){alert('配置文件过大');return}
  fetch('/api/config',{method:'POST',body:txt,headers:{'X-Auth-Token':_authToken}}).then(function(r){return r.text()}).then(function(t){
    alert(t);closeCfg();
...
def render():
    ...
    for p in peers:
        # 安全：HTML转义所有动态内容
        nat_cn = NAT_CN.get(p["nat"], html_mod.escape(p["nat"]))
        if p["cost"] == "Local":
        ...
        rows.append(
            f'<tr{cls}><td class="ip" onclick="copyTxt(this)">{html_mod.escape(p["ipv4"])}</td>'
            f'<td class="host">{html_mod.escape(p["hostname"])}</td><td>{tag}</td>'
            f'<td class="nc">{html_mod.escape(p["latency"])}</td><td class="nc">{html_mod.escape(p["loss"])}</td>'
            f'<td class="nc">{html_mod.escape(p["rx"])}</td><td class="nc">{html_mod.escape(p["tx"])}</td>'
            f'<td>{html_mod.escape(p["tunnel"])}</td><td>{nat_cn}</td>'
            f'<td class="vr">{html_mod.escape(p["version"])}</td></tr>'
        )

    rrows = [
        f'<tr><td class="ip">{html_mod.escape(r["cidr"])}</td><td>{html_mod.escape(r["hostname"])}</td>'
        f'<td>{html_mod.escape(r["hop"])}</td><td class="nc">{html_mod.escape(r["cost"])}</td></tr>'
        for r in routes
    ]

    listeners_html = "".join(
        f'<span class="listener">{html_mod.escape(l)}</span>' for l in node.get("listeners", [])
    )
    ...
    def do_GET(self):
        if self.path == "/api/config":
            ...
            if not _check_auth(self):
                ...
                _log(f"API认证失败: {self.client_address}")
                return
            try:
                with open(CONFIG_TOML, "r", encoding="utf-8") as f:
                    content = f.read()
        ...
        if self.path == "/api/config":
            ...
            try:
                ...
                if length > 100_000:
                    ...
                    self.end_headers()
                    self.wfile.write(b"Config too large")
                    return
                ...
                if "[network_identity]" not in body and "[flags]" not in body:
                    ...
                    self.end_headers()
                    self.wfile.write(b"Invalid config format")
                    return
                ...
                _log(f"配置已更新: {self.client_address}")
            except Exception as e:
                _log(f"保存配置失败: {e}")
                self.send_response(500)
                self.end_headers()
                # 安全：不泄露内部错误信息
                self.wfile.write("保存失败，请检查配置格式".encode("utf-8"))
...
if __name__ == "__main__":
    os.chdir(_here)
    _log(f"Dashboard v5 启动: http://127.0.0.1:{PORT}")
