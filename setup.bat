...
echo %NET_NAME%| findstr /R "[<>|&]" >nul 2>&1 && ( echo   ✗ 网络名称不能包含特殊字符 & pause & exit /b 1 )

set /p "NET_SECRET=*** "
if "%NET_SECRET%"=="" ( echo   ✗ 不能为空 & pause & exit /b 1 )
if "%NET_SECRET:~3%"=="" ( echo   ✗ 密钥至少4个字符 & pause & exit /b 1 )

set /p "SERVER_URI=  引导节点 [如 tcp://1.2.3.4:11010]: "
if "%SERVER_URI%"=="" ( echo   ✗ 不能为空 & pause & exit /b 1 )
echo %SERVER_URI%| findstr /R "^tcp:// ^udp://" >nul 2>&1 || ( echo   ✗ 地址必须以 tcp:// 或 udp:// 开头 & pause & exit /b 1 )

set /p "CIDR_PREFIX=  网段 [默认 10.0.0]: "
if "%CIDR_PREFIX%"=="" set "CIDR_PREFIX=10.0.0"
echo %CIDR_PREFIX%| findstr /R "^[0-9]*\.[0-9]*\.[0-9]*$" >nul 2>&1 || ( echo   ✗ 网段格式错误，需要 X.X.X 格式 & pause & exit /b 1 )
...
set /p "VIP=  IP末位: "
if "%VIP%"=="" set "VIP=2"
if %VIP% LSS 2 ( echo   ✗ IP末位必须 >= 2 & pause & exit /b 1 )
if %VIP% GTR 254 ( echo   ✗ IP末位必须 <= 254 & pause & exit /b 1 )
